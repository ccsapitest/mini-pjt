...
def _exact_match(query: str) -> dict | None:
    ...
    알려진 한계: "긴 키워드 우선"이 항상 옳지는 않다 — 예를 들어 "쑝쑝돈까스 에비덴 왕새우
    커리"처럼 식당 이름("쑝쑝돈까스")이 메뉴명 앞에 섞여 들어오면, 진짜 메뉴는 "커리"(→카레)
    인데 식당명 안에 우연히 들어있는 "돈까스"(3자)가 "커리"(2자)보다 길어서 잘못 매칭된다.
    호출하는 쪽(agent.py 프롬프트)에서 식당명을 메뉴명과 섞어 넘기지 않도록 안내해 실제
    발생을 막고 있지만, 이 함수 자체는 여전히 그런 입력이 오면 틀릴 수 있다.
    """
